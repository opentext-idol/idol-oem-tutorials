...
namespace fetchSynchronize
{
    class Program
    {
        ...
        {
            Console.WriteLine("Usage: FetchSynchronize <HOSTNAME/IP> <ACI_PORT_#> <TASK_NAME>\n");
        ...
        static void FetchSynchronize(String[] args)
        {
            ...
            Command getStatus = new Command("FETCH");
            //Command getStatus = new Command("INGEST");
            getStatus.Set("fetchAction", "synchronize");
            getStatus.Set("configSection", args[2]);
            getStatus.Set("responseFormat", "xml");

            Console.WriteLine("\nExecuting action...");

            try
            {
                response = connection.Execute(getStatus);
            }
            catch (Exception e)
            {
