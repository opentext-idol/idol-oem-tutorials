...
{
	printf("Usage: ms_process <HOSTNAME/IP> <ACI_PORT_#> <CONFIG_NAME> <SOURCE>\n");
	printf("Parameters:\n");
	printf("    <HOSTNAME/IP>		Hostname / IP address of ACI service.\n");
	printf("    <ACI_PORT_#>		ACI port number of ACI service.\n");
	printf("    <CONFIG_NAME>		Media Server session configuration name.\n");
	printf("    <SOURCE>		Media source file path to process.\n");
}
...
int main(int argc, char** argv)
{
	...
	aciObjectParamSetString(pCommand, "configName", argv[3]);
	aciObjectParamSetString(pCommand, "source", argv[4]);
