...
{
	printf("Usage: fetch_collect <HOSTNAME/IP> <ACI_PORT_#> <IDENTIFIERS_CSV> <DESTINATION_FOLDER>\n");
	printf("Parameters:\n");
	printf("    <HOSTNAME/IP>		Hostname / IP address of ACI service.\n");
	printf("    <ACI_PORT_#>		ACI port number of ACI service.\n");
	printf("    <IDENTIFIERS_CSV>		CSV of identifiers.\n");
	printf("    <DESTINATION_FOLDER>	Destination for collected data.\n");
}
...
int main(int argc, char** argv)
{
	...
	aciObjectParamSetString(pCommand, "fetchAction", "collect");
	aciObjectParamSetString(pCommand, "identifiers", argv[3]);
	aciObjectParamSetString(pCommand, "destination", argv[4]);
