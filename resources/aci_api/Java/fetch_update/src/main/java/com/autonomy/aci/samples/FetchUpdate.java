...
public class FetchUpdate {
    ...
    private static final Logger LOG = Logger.getLogger(FetchUpdate.class.getName());
	...
	static class Args {
        static final int EXPECTED_NUM_ARGS = 3;
		static final int MAX_IDENTIFIERS_XML_SIZE_BYTES = 10000;

		final String aciHost;
		final Integer aciPort;
		final String identifiersXML_file;
		
		//Prints program usage information to standard output
		private void displayusageinfo()
		{
			System.console().printf("Usage: FetchUpdate <HOSTNAME/IP> <ACI_PORT_#> <IDENTIFIERS_XML_FILE>\n");
			...
			System.console().printf("    <IDENTIFIERS_XML_FILE>	Update identifiers XML file.\n");
		}
		...
		Args(final String[] stringArgs) throws IllegalArgumentException {
			...
			identifiersXML_file = stringArgs[2];

			System.console().printf("ACI Host: %s\n", aciHost);
			System.console().printf("ACI Port: %d\n", aciPort);
			System.console().printf("IdentifiersXML file: %s\n", identifiersXML_file);
		}
	}

	private String get_identifiersXML_file(String pathStr) throws Exception
	{
		String identifiersXML;

		try {
			identifiersXML = new String(Files.readAllBytes(Paths.get(pathStr)));
		} catch(Exception e) {
    ...
    private void fetch_update(String[] args) throws Exception
	{
		Args parsedArgs;
		...
		try {
			...
			String identifiersXML = get_identifiersXML_file(parsedArgs.identifiersXML_file);
			
			// Set action parameters
			parameters = new ActionParameters("FETCH");
			parameters.add("fetchAction", "update");
			parameters.add("identifiersXML", identifiersXML);
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        FetchUpdate obj = new FetchUpdate();

        try {
            obj.fetch_update(args);
        } catch(Exception e) {
