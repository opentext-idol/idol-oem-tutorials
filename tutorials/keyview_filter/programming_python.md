...
To create a program that uses the KeyView Filter SDK for Python, you need to install a supported Python 3 interpreter.  The `%KEYVIEW_HOME%\pythonapi\keyview\*.whl` has encoded in the filename the Python version number (*e.g.* `*-cp310-*` for v3.10) supported.  Reference the documentation section [Install the KeyView Filter Python Module](https://www.microfocus.com/documentation/idol/IDOL_24_3/KeyviewFilterSDK_24.3_Documentation/Guides/html/python-programming/Content/Python/InstallModule.htm) for installation tips.
...
```sh
D:\_Opentext\_Code\_MF_GitHub\idol-oem-tutorials\resources\keyview_filter\sdk\python>python extract_metadata_text.py
...
> NOTE: The `run_extract_*.*` scripts set `KV_SAMPLE_PROGRAM_LICENSE_FROM_FILEPATH` and assume you deploy the `licensekey.dat` to `%KEYVIEW_HOME%`.

```sh
set KV_SAMPLE_PROGRAM_LICENSE_FROM_FILEPATH=C:\OpenText\KeyViewFilterSDK_24.3.0_WINDOWS_X86_64\licensekey.dat
```
 ...
 ```json
    ...
    "launch": {
        "configurations": [
        {
            ...
            "program": "${file}",
            "env": {
                "KV_SAMPLE_PROGRAM_LICENSE_FROM_FILEPATH": "C:\\OpenText\\KeyViewFilterSDK_24.3.0_WINDOWS_X86_64\\licensekey.dat"
            }
        }
        ]
    }
...
Reference the [Use the Filter API](https://www.microfocus.com/documentation/idol/IDOL_24_3/KeyviewFilterSDK_24.3_Documentation/Guides/html/cpp-programming/Content/Chapter_UseFilterAPI.htm) section of the KeyView Filter SDK Programming Guide.
...
...
Using KeyView license key: C:\OpenText\KeyViewFilterSDK_24.3.0\licensekey.dat
---------------
```

A file called `extract_sub_files.LOG` will be generated containing output as below:
```txt
KeyViewing file: D:\_OpenText\KeyView\FilterSDK_24.3.0\javaapi\KeyView.jar
...
to D:\_Opentext\_Code\_MF_GitHub\idol-oem-tutorials\_WORK\output\META-INF\MANIFEST.MF
	Type: Main
	Index: 1
	Parent: 0
	Size (bytes): 104
...
KeyViewing file: D:\_Opentext\_Code\_MF_GitHub\idol-oem-tutorials\_WORK\output\META-INF\MANIFEST.MF
...
to D:\_Opentext\_Code\_MF_GitHub\idol-oem-tutorials\_WORK\output\com\verity\api\Charset.class
	Type: Main
	Index: 9
	Parent: 4
	Size (bytes): 7208
